...
void economic_analysis(PASSENGER *passengers)
{
    ...
    {
        if (k == 7)
            printf(cr_green "Groupo %d ( cheio )  {" cr_reset "%3d" cr_green "}  |" cr_reset, k+1, info[k]);
        else
            printf(cr_green "Groupo %d (%2d - %2d)  {" cr_reset "%3d" cr_green "}  |" cr_reset, k+1, k*3, k*3+2, info[k]);

        for(i=0;i<30*(((double)info[k])/largest);i++) // Floors the decimal places.
            putchar('*');
